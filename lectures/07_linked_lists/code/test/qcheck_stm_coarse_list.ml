(** QCheck-STM State Machine Test for CoarseList

    This test uses QCheck-STM to verify the coarse-grained list's correctness.
    The list uses a single mutex, so it should be safe for all concurrent
    access patterns.
...
module Spec = struct
  ...
  let run c d =
    ...
    | Add i ->
        Res (bool, CL.add d i)
    | Remove i ->
        Res (bool, CL.remove d i)
    | Contains i ->
        Res (bool, CL.contains d i)
...
let () =
  let test_name =
    if Array.length Sys.argv > 1 then
      ...
      "all"
  in
  Printf.printf "QCheck-STM Tests for CoarseList\n\n";
