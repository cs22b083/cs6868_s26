(** QCheck-STM State Machine Test for LockFreeList

    This test uses QCheck-STM to verify the lock-free list's correctness.
    The list uses atomic operations without any locks, so it should be
    safe for all concurrent access patterns.

    Tests demonstrate:
    - Sequential operations work correctly
    - Concurrent operations (multiple domains) are safe
    - Lock-free operations scale well under contention
*)
...
module Spec = struct
  ...
  let arb_cmd _s =
    ...
    QCheck.make ~print:show_cmd
      (Gen.frequency [
        ...
        (3, Gen.map (fun i -> Contains i) int_gen);  (* Lock-free contains *)
      ])

  let init_state = []
  let init_sut () = LF.create ()
  let cleanup _ = ()
  ...
  let run c d =
    ...
    | Add i ->
        Res (bool, LF.add d i)
    | Remove i ->
        Res (bool, LF.remove d i)
    | Contains i ->
        Res (bool, LF.contains d i)
...
let run_test test_name =
  ...
  | "sequential" | "seq" ->
      ...
      let seq_test = Seq.agree_test ~count:5000 ~name:"LockFreeList sequential" in
  ...
  | "concurrent" | "conc" ->
      ...
      in
      let conc_test =
        QCheck.Test.make ~retries:10 ~count:200 ~name:"LockFreeList concurrent" arb_cmds_par
        @@ fun triple ->
  ...
  | "all" ->
      ...
      let tests = [
        Seq.agree_test ~count:5000 ~name:"LockFreeList sequential";
        QCheck.Test.make ~retries:10 ~count:200 ~name:"LockFreeList concurrent"
          (Dom.arb_triple 12 8 Spec.arb_cmd Spec.arb_cmd Spec.arb_cmd)
...
let () =
  let test_name =
    if Array.length Sys.argv > 1 then
      ...
      "all"
  in
  Printf.printf "QCheck-STM Tests for LockFreeList\n\n";
