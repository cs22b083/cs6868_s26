(** QCheck-STM State Machine Test for OptimisticList

    This test uses QCheck-STM to verify the optimistic list's correctness.
    The list uses optimistic locking with validation, so it should be safe
...
module Spec = struct
  ...
  let run c d =
    ...
    | Add i ->
        Res (bool, OL.add d i)
    | Remove i ->
        Res (bool, OL.remove d i)
    | Contains i ->
        Res (bool, OL.contains d i)
...
let run_test test_name =
  ...
  | "sequential" | "seq" ->
      ...
      let seq_test = Seq.agree_test ~count:5000 ~name:"OptimisticList sequential" in
  ...
  | "concurrent" | "conc" ->
      ...
      in
      let conc_test =
        QCheck.Test.make ~retries:10 ~count:200 ~name:"OptimisticList concurrent" arb_cmds_par
        @@ fun triple ->
  ...
  | "all" ->
      ...
      let tests = [
        Seq.agree_test ~count:5000 ~name:"OptimisticList sequential";
        QCheck.Test.make ~retries:10 ~count:500 ~name:"OptimisticList concurrent"
          (Dom.arb_triple 12 8 Spec.arb_cmd Spec.arb_cmd Spec.arb_cmd)
...
let () =
  let test_name =
    if Array.length Sys.argv > 1 then
      ...
      "all"
  in
  Printf.printf "QCheck-STM Tests for OptimisticList\n\n";
