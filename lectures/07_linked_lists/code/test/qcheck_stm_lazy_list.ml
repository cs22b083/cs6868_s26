(** QCheck-STM State Machine Test for LazyList

    This test uses QCheck-STM to verify the lazy list's correctness.
    The list uses lazy synchronization with wait-free contains,
    so it should be safe for all concurrent access patterns.

    Tests demonstrate:
    - Sequential operations work correctly
    - Concurrent operations (multiple domains) are safe
    - Wait-free contains doesn't interfere with add/remove
*)
...
module Spec = struct
  ...
  let run c d =
    ...
    | Add i ->
        Res (bool, LL.add d i)
    | Remove i ->
        Res (bool, LL.remove d i)
    | Contains i ->
        Res (bool, LL.contains d i)
...
let run_test test_name =
  ...
  | "sequential" | "seq" ->
      ...
      let seq_test = Seq.agree_test ~count:5000 ~name:"LazyList sequential" in
  ...
  | "concurrent" | "conc" ->
      ...
      in
      let conc_test =
        QCheck.Test.make ~retries:10 ~count:200 ~name:"LazyList concurrent" arb_cmds_par
        @@ fun triple ->
  ...
  | "all" ->
      ...
      let tests = [
        Seq.agree_test ~count:5000 ~name:"LazyList sequential";
        QCheck.Test.make ~retries:10 ~count:500 ~name:"LazyList concurrent"
          (Dom.arb_triple 12 8 Spec.arb_cmd Spec.arb_cmd Spec.arb_cmd)
          (fun triple -> QCheck.assume (Dom.all_interleavings_ok triple); repeat 20 Dom.agree_prop_par triple);
      ] in
...
let () =
  let test_name =
    if Array.length Sys.argv > 1 then
      ...
      "all"
  in
  Printf.printf "QCheck-STM Tests for LazyList\n\n";
