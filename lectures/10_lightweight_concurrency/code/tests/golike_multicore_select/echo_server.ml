...
(** Per-connection echo handler: reads until EOF and echoes each chunk back. *)
...
let test_single_echo () =
  Sched.run ~num_domains:4 (fun () ->
      ...
      (* Server: accept one connection, handle it, then close listener *)
      Sched.fork (fun () ->
...
let test_multi_client_echo () =
  Sched.run ~num_domains:4 (fun () ->
      ...
      (* Server: accept two connections *)
      Sched.fork (fun () ->
          for _ = 1 to 2 do
            let cfd, _addr = Io.accept server in
            Sched.fork (fun () -> echo_handler cfd)
          done;
          (* Wait for both clients to finish before closing server *)
          Chan.recv done_ch; Chan.recv done_ch;
          Unix.close server);
      (* Spawn two client fibers *)
      let run_client id =
        let client = Unix.socket ~cloexec:true Unix.PF_INET Unix.SOCK_STREAM 0 in
        Unix.set_nonblock client;
        Io.connect client (Unix.ADDR_INET (Unix.inet_addr_loopback, port));
...
let () =
  ...
  Printf.printf "echo_server: PASSED\n%!"
